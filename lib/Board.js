...
Board.checkWallCollision = function(ball){
  var collision = {};
  collision.maxX = BOARD_WIDTH - ball.radius;
  collision.maxY = BOARD_HEIGHT - ball.radius;
  if ( ball.position.x < ball.radius || ball.position.x > collision.maxX ) collision.horizontal = true;
  if ( ball.position.y < ball.radius || (ball.position.y > collision.maxY && ball.velocity.y > 0) ) collision.vertical = true;
